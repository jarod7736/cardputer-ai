...
// Font: text size 1 (6×8 px) on a 240×135 LCD = 40 cols × 16 rows.
// Top row: status bar. Bottom row: input. Middle 14 rows: scrollback.
...
static void draw_status_row() {
  M5Cardputer.Display.fillRect(0, kStatusRow * kCharH,
                                M5Cardputer.Display.width(), kCharH, TFT_NAVY);
  M5Cardputer.Display.setTextColor(TFT_WHITE, TFT_NAVY);
  M5Cardputer.Display.setCursor(0, kStatusRow * kCharH);
...
static void draw_scrollback() {
  ...
  M5Cardputer.Display.fillRect(0, y, M5Cardputer.Display.width(),
                                kScrollbackRows * kCharH, TFT_BLACK);
...
static void draw_input_row() {
  ...
  M5Cardputer.Display.fillRect(0, y, M5Cardputer.Display.width(), kCharH, TFT_BLACK);
  M5Cardputer.Display.setTextColor(TFT_WHITE, TFT_BLACK);
  M5Cardputer.Display.setCursor(0, y);
  M5Cardputer.Display.print("> ");
  M5Cardputer.Display.print(s_input_buf);
  // Cursor as a 1-px underline.
  uint16_t cur_x = (uint16_t)(s_cursor + 2) * kCharW;
  if (cur_x < M5Cardputer.Display.width()) {
    M5Cardputer.Display.fillRect(cur_x, y + kCharH - 1, kCharW - 1, 1, TFT_WHITE);
  }
}
